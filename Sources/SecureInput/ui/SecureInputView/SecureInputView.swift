 ...
 * - Description: A SwiftUI view for secure single-line password fields, suitable for authentication and grant purposes. It does not work with vertical axis text fields and may clear text when toggling from unsecured to secure mode. This view is used in `MasterPassView` and provides notes on secure input, cursor management, and potential improvements.
 ...
 * - Important: ⚠️️ The text will be cleared if the user continue to write in secure mode after toggeling from unsecured mode. This isnt such an issue when used in auth / verify prompts, but can be if this field is to store a password etc.
 * - Note: Used in: `MasterPassView`
 * - Note: Discusses secure input: https://stackoverflow.com/questions/63095851/show-hide-password-how-can-i-add-this-feature
 * - Note: using opacity avoids resetting text when toggle, but cursor pointer is lost, minor annoyance
 * - Note: Using `focusState` avoids cursor loss, but resets input, no go
 * - Note: To avoid keyboard loss between toggles see: https://softwareanders.com/swiftui-password-show-and-hide/
 * - Note: Supports showing the last chracter you type: (very advance solution) https://github.com/perfaram/TogglableSecureField
 ...
 * - Note: This component is utilized in views requiring secure text entry, such as login, registration, and password update forms within the ExampleApp.
 */
