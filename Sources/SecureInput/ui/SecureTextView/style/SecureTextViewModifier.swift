import SwiftUI
/**
 * - Description: This modifier is used to secure text input by replacing 
 *                characters with a placeholder character, such as '●', to 
 *                conceal the actual input. It dynamically updates the 
 *                visible input based on the user's input and the security 
 *                state.
...
fileprivate struct SecureTextViewModifier: ViewModifier {
    ...
    *                represents the text that is currently visible in the 
    *                text field. If the text is secured, this string will 
    ...
    *                whether the text in the text field is obscured. When 
    *                true, the text is displayed as dots or asterisks to 
    ...
    *                stores the actual user input. It is used to keep 
    *                track of the text entered by the user, regardless 
    ...
    *                listening for changes in the visible input. If the 
    *                input is secured, it replaces each character with a 
    *                '●' character to obscure the actual input. It also 
    *                manages the inputValue based on the changes in the 
    *                visible input.
    * 1. Listen for changes to the visibleInput and capture the new value.
    * 2. If the input is not secured, update inputValue and exit the 
    *    onChange closure.
    * 3. If new input is longer than or equal to current inputValue, 
    *    filter out placeholder characters and append new characters 
    *    to inputValue.
    * 4. If new input is shorter, remove the last character from 
    *    inputValue.
    * 5. Replace each character in newValue with a placeholder 
    *    character to obscure the actual input.
...
extension View {
   /**
    * - Description: Applies the SecureTextModifier to a view, managing 
    *               the visibility and security of text input.
    * - Parameters:
    *   - visibleInput: The text visible to the user in the TextField.
    *   - isSecured: A boolean to toggle between concealed and revealed 
    *                text.
